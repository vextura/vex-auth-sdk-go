# vex-auth-sdk-go

Typed Go SDK for **vex-auth**, generated from the canonical Smithy spec.

```bash
go get github.com/vextura/vex-auth-sdk-go
```

## Usage

```go
import "github.com/vextura/vex-auth-sdk-go"

func main() {
    client := auth.New("https://gate.example.com", "<bearer-token>")
    // typed method per Smithy operation
}
```

## Generated

This package is generated by [`vexctl clientgen`](https://github.com/vextura/vexctl). Do not edit the generated files (`client.go`, `types.go`, `operations.go`, `errors.go`, `doc.go`) by hand — regenerate from the Smithy source when the API changes.

## License

Apache 2.0 — see [LICENSE](./LICENSE).
